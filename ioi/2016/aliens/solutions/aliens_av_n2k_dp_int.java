...
public class aliens_av_n2k_dp_int {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < k; j++) {
                for (int next = i; next < n; next++) {
                    int cost = sqr(ranges[next].end - ranges[i].start + 1);
    ...
    void run() {
        ...
        long ans = new aliens_av_n2k_dp_int().take_photos(n, m, k, r, c);
