...
public class railroad_wa {
    ...
    public long plan_roller_coaster(int[] s, int[] t) {
        ...
        for (int i = 0, delta = 0; i + 1 < e.size(); ++i) {
            delta += e.get(i).type;
            res += Math.max(0, delta) * (long) (e.get(i + 1).c - e.get(i).c);
            edges.add(new Edge(e.get(i + 1).c - e.get(i).c, e.get(i).num, e.get(i + 1).num));
            if ((e.get(i + 1).c == e.get(i).c) || (delta != 1))
    ...
    private void run() {
        ...
        long ans = new railroad_wa().plan_roller_coaster(s, t);
