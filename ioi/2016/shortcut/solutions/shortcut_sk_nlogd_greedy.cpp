...
ll find_shortcut(int n, vector<int> len, vector<int> d, int c) {
	...
	ll mi = *max_element(all(d)) - 1, ma = 2 * *max_element(all(d)) + accumulate(all(len), 0LL);
	while (ma - mi > 1) {
		ll x = (ma + mi) / 2;
		int l = 0, r = n - 1;
		while (l + 1 < n && pref_max[l + 1] <= x && pref_end[l + 1] * 2 <= x - c)
			l++;
		while (r - 1 >= 0 && suf_max[r - 1] <= x && suf_end[r - 1] * 2 <= x - c)
			r--;
		bool good = (l >= r);
		const int M = 10;
		forab(dl, -M, M)
			forab(dr, -M, M)
				good = good || (calc(l + dl, r + dr, x + 1) <= x);
		(good ? ma : mi) = x;
	}
	return ma;
}
