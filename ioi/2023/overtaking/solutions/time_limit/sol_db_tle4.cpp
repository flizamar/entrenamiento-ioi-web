...
ll calc(int j, ll y) {
    ...
    int nxtj = j + 1;
    ll curt = y;
    nxtj = max(nxtj, nextj[j][l]);

    while (nxtj > j + 1 && (l? premax[nxtj][l - 1]: 0) >= y + 1ll * x * (s[nxtj] - s[j])) {
        nxtj--;
    }
    ll res;
    while (nxtj < m) {
        curt = max(y + 1ll * x * (s[nxtj] - s[j]), l? premax[nxtj][l - 1]: 0);
        if (curt == (l? premax[nxtj][l - 1]: 0)) {
            nextj[j][l] = nxtj;
            res = calc(nxtj, curt);
            break;
        }
