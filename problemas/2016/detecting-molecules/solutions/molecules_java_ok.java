 ...
 */
public class molecules_java_ok {
    public static void main(String[] args) {
        InputStream inputStream = System.in;
        OutputStream outputStream = System.out;
        InputReader in = new InputReader(inputStream);
        OutputWriter out = new OutputWriter(outputStream);
        Molecules solver = new Molecules();
        solver.solve(1, in, out);
        out.close();
    }

    static class Molecules {
        ...
        int[] solve(int l, int u, int[] w) {
            int[] order = ArrayUtils.createOrder(w.length);
            ArrayUtils.orderBy(w, order);
            int qty = 0;
            long sum = 0;
            ...
            int[] answer = ArrayUtils.createOrder(qty);
            for (int i = 0; i < qty && low < l; i++) {
                answer[i] = w.length - i - 1;
                low += w[w.length - i - 1] - w[i];
            }
            for (int i = 0; i < qty; i++) {
                answer[i] = order[answer[i]];
            }
            return answer;
        }
