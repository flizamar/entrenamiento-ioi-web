 ...
 * @author: pashka
 */
public class paint_borders_only2 {
    ...
    public String solve_puzzle(String S, int[] C) {
        ...
        Arrays.fill(ans, '_');
        for (int i = 0; i < C.length; i++) {
            for (int j = leftmost[i]; j < rightmost[i] + C[i]; j++) {
                ans[j] = '?';
            }
        }
        for (int i = 0; i < C.length; i++) {
    ...
    private void run() throws IOException {
        ...
        String ans = new paint_borders_only2().solve_puzzle(S, C);
