...
class shortcut {
    ...
    public void solve(int testNumber, shortcut_java_ok.InputReader in, shortcut_java_ok.OutputWriter out) {
        int n = in.readInt();
        int c = in.readInt();
        int[] l = shortcut_java_ok.IOUtils.readIntArray(in, n - 1);
        int[] d = shortcut_java_ok.IOUtils.readIntArray(in, n);
    ...
    boolean can(long limit) {
        ...
        int special = Arrays.binarySearch(interesting, maxPos);
        int down = special == 0 ? -1 : interesting[special - 1];
        for (int i = maxPos - 1; i > down; i--) {
        ...
        int up = special == interesting.length - 1 ? n : interesting[special + 1];
        for (int i = maxPos + 1; i < up; i++) {
            if (d[i] + d[maxPos] - pos[maxPos] + pos[i] <= limit) {
                continue;
            }
            long dist = d[i] + d[maxPos] + c;
    ...
    long find_shortcut(int n, int[] l, int[] d, int c) {
        ...
        shortcut_java_ok.IntList list = new shortcut_java_ok.IntArrayList();
        for (int i = 0; i < n; i++) {
            if (viable[i]) {
                list.add(i);
            }
        }
        maxPos = shortcut_java_ok.ArrayUtils.maxPosition(d);
        interesting = list.toArray();
        long left = shortcut_java_ok.ArrayUtils.maxElement(d);
        long right = shortcut_java_ok.ArrayUtils.sumArray(l) + 2 * shortcut_java_ok.ArrayUtils.maxElement(d);
        while (left < right) {
